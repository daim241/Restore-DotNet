...
import { useCreaterOrderMutation } from "../orders/orderApi";

const steps = ['Address', 'Payment', 'Review'];

export default function CheckoutStepper() {
    const [acticveStep, setActiveStep] = useState(0);
    const[createOrder] = useCreaterOrderMutation();
    ...
    const handleNext = async () => {
        if (acticveStep === 0 && saveAddressChecked && elements) {
        ...
        if (acticveStep < 2) setActiveStep(step => step + 1);
    }
    ...
    return (
        <Paper sx={{ p: 3, borderRadius: 3 }}>
            <Stepper activeStep={acticveStep}>
            ...
            <Box sx={{ mt: 2 }}>
                <Box sx={{ display: acticveStep === 0 ? 'block' : 'none' }}>
                    <AddressElement
                        options={{
                            mode: 'shipping',
                            defaultValues: {
                ...
                <Box sx={{ display: acticveStep === 1 ? 'block' : 'none' }}>
                    <PaymentElement
                ...
                <Box sx={{ display: acticveStep === 2 ? 'block' : 'none' }}>
            ...
            <Box display='flex' paddingTop={2} justifyContent='space-between'>
                ...
                <LoadingButton
                    onClick={handleNext}
                    disabled={
                        (acticveStep === 0 && !addressComplete) ||
                        (acticveStep === 1 && !paymentComplete) ||
                        submitting
                    }
                    loading={submitting}
                >
                    {acticveStep === steps.length - 1 ? `Pay ${currencyFormat(total)}` : 'Next'}
                </LoadingButton>
            </Box>
        </Paper>
    )
