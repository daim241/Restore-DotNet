...
public class PaymentsController(PaymentsServices paymentsService,
    ...
    : BaseApiController
{
        ...
        foreach (var item in order.OrderItems)
        {
            var productItem = await context.Products
                .FindAsync(item.ItemOrdered.ProductId)
                    ?? throw new Exception("Problem updatig order stock");
    ...
    private async Task HandlePaymentIntentSucceeded(PaymentIntent intent)
    {
        ...
        if (order == null)
        {
            logger.LogWarning("Order not found for PaymentIntent: {Id}", intent.Id);
            return; // Prevents the 500 error from a null reference below
        }
